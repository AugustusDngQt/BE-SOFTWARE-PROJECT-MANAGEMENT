import { type Sprints, type Issues, type Comments } from '@prisma/client';

export function generateInitialUserComments(userId: string): Comments[] {
  const now = new Date();
...
export function generateInitialUserSprints(userId: string): Sprints[] {
  const now = new Date();
...
export function generateInitialUserIssues(userId: string): Issues[] {
  const now = new Date();
  const slicedUserId = userId ? userId.slice(5, 12) : 'init';
